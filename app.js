...
app.post('/api/facedetect', function(req, res) {

  var vrParams = {
    image_file: req.files.sampleFile.data
  };
  var myJSON = '';
  var actions = [];

  var fn = visualRecognition.detectFaces(vrParams,
    function (err, vrResponse) {
      if (err)
        console.log(err);
      else
        myJSON = JSON.stringify(vrResponse, null, 2);
        if (vrResponse.images[0].faces.length == 0) {
          res.send("{\"facedetect\" : false}");
        } else { //we have at least one face found
          res.send("{\"facedetect\" : true}");
        }
